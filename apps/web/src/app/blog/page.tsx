...
function FeaturedHero({ post }: { post: BlogPost }) {
  return (
    <Link href={`/blog/${post.slug}`} className="relative rounded-2xl overflow-hidden aspect-[4/3] group cursor-pointer block">
      {/* gradient "image" */}
      <div
...
function FeaturedRow({ post }: { post: BlogPost }) {
  return (
    <Link href={`/blog/${post.slug}`} className="flex items-center gap-4 group cursor-pointer">
      {/* thumbnail */}
      <div
